...
var app = new Vue({
    ...
    data: {
        age: age,
        currentRole: 'fullstack', // Default role
        isAdminMode: window.location.search.includes('?admin'),
        isRoleSelectorVisible: false,
        cvData: {
            devops: {
                title: "DevOps",
                profile: "Développeur passionné avec une forte appétence pour le DevOps. J'aime concevoir des architectures scalables, automatiser les déploiements et garantir la stabilité des systèmes. Autodidacte et curieux, je suis toujours à la recherche de nouveaux défis.",
                experiences: [
                    {
            ...
            fullstack: {
                ...
                profile: "Développeur Full Stack curieux et polyvalent, j'apprécie la création d'applications web complètes, de la conception des bases de données à l'implémentation des interfaces utilisateur. Motivé par l'apprentissage et les défis techniques, je m'efforce de livrer des solutions fonctionnelles et bien pensées.",
                experiences: [
                    {
                        society: "Reymann / Sprey, Illkirch, France",
                        date: "Juin 2019 - Septembre 2025",
    ...
    computed: {
        currentCvData() {
            return this.cvData[this.currentRole];
        },
        sortedSkills() {
            // Make sure skills is not undefined
            if (!this.currentCvData || !this.currentCvData.skills) {
                return [];
            }
            return [...this.currentCvData.skills].sort((a, b) => b.level - a.level);
        }
    }
});